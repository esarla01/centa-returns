...
interface FaultResponsibilityData {
  fault_responsibility: string;
  item_count: number;
  percentage: number;
}

interface FaultResponsibilityResponse {
  total_items: number;
  data: FaultResponsibilityData[];
}

interface FaultResponsibilityChartProps {
  startDate: Date | null;
  endDate: Date | null;
  refreshKey: number;
}
...
export default function FaultResponsibilityChart({
  startDate,
  endDate,
  refreshKey,
}: FaultResponsibilityChartProps) {
  const [data, setData] = useState<FaultResponsibilityData[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const result: FaultResponsibilityResponse = await response.json();
        setData(result.data);
    ...
    <div className="w-full">
      ...
      <ResponsiveContainer width="100%" height={280}>
        <PieChart>
          <Pie
            ...
            dataKey="item_count"
            labelLine={false}
            label={({ fault_responsibility, item_count, percentage }) => 
              `${item_count} adet`
            }
            outerRadius={70}
            innerRadius={20}
      ...
      <div className="mt-4 grid grid-cols-1 gap-2">
        {chartData.map((item, index) => (
          <div key={item.fault_responsibility} className="flex items-center justify-between text-sm">
            <div className="flex items-center">
              <div
                className="w-3 h-3 rounded-full mr-2"
                style={{ backgroundColor: COLORS[index % COLORS.length] }}
              />
              <span className="text-gray-700">{item.fault_responsibility}</span>
            </div>
