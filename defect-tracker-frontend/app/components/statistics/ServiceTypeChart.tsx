    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          buildApiUrl(API_ENDPOINTS.REPORTS.SERVICE_TYPE_STATS) + `?start_date=${startDateStr}&end_date=${endDateStr}`,
          {
            credentials: 'include',
          }
        );
        ...
        const result: ServiceTypeResponse = await response.json();
        setData(result.data);
      ...
      <ResponsiveContainer width="100%" height={280}>
        <PieChart>
          <Pie
            ...
            dataKey="item_count"
            labelLine={false}
            label={({ service_type, item_count, percentage }) => 
              `${item_count} adet`
            }
            outerRadius={70}
            innerRadius={20}
          ...
          <Tooltip
            ...
            labelFormatter={(label: string) => `Hizmet Türü: ${label}`}
            contentStyle={{
