...
import CaseFilter from '@/components/Filters';
...
export default function MainPage() {
  ...
  const fetchCases = async () => {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:5000/api/retrieve-cases?${queryParams}`, {
        method: 'GET',
              ...
              <div className="mt-4 flex justify-end">     
                <EditButton
                  ...
                  onAdd={async (editedCase) => {
                    try {
                      const response = await fetch('http://127.0.0.1:5000/api/update-case', {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json' },
